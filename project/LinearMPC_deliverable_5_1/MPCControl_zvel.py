...
class MPCControl_zvel(MPCControl_base):
    ...
    u_target: np.ndarray = None
        ...
        # keep as column vectors (1,1) to avoid scalar/shape issues
        ...
        if self._est_initialized ==False:
            ...
            # ---------- observer update (augmented Luenberger) ----------
            # Project disturbance model: x^+ = A x + B u + B d   (Bd = B)
            ...
            # gains (store these in _setup_controller; fall back if missing)
        ...
        # ---------- MPC parameters ----------
        # dx0_p and d_p were created as cp.Parameter((1,)) in _setup_controller,
        # so we must pass shape (1,), not (1,1).
        dx0 = self._xhat.reshape(1,)                             # (1,)
        dhat = self._dhat.reshape(1,)                            # (1,)

        dxt = np.array(x_target - self.xs, dtype=float).reshape(1,)   # (1,)
        # Offset-free: shift the steady input target by -dhat
        ...
        if self.ocp.status not in ("optimal", "optimal_inaccurate"):
            ...
            # clip to delta input bounds if you stored them
            ...
            # provide trajectories (simple placeholders)
        ...
        print("vz:", float(y), "dhat:", float(self._dhat), "Pavg:", float(u0))
                      # (1,)
